...
# schema gate_out stream
gate_out_schema = StructType([
...
@dp.table(
    name="gate_out_events_bronze",
    comment="Raw gate out container events from Kafka",
    temporary=False
)
def gate_out_bronze():
    return(
        ...
        .option("subscribe","gate_out_events")
...
# buat delta table untuk silver gate out events
@dp.table(
    name="gate_out_events_silver",
    comment="Transformed gate out container events with flattened structure for dwell time and status tracking",
    temporary=False
)
def gate_out_silver():
    return(
        spark.readStream
            .table("gate_out_events_bronze")
            .selectExpr("CAST(value AS STRING) as json", "timestamp as kafka_timestamp", "topic")
            .select(
                from_json(col("json"), gate_out_schema).alias("data"),
                col("kafka_timestamp"),
                col("topic")
            )
            .select(
                col("data.event_id"),
                to_timestamp(col("data.timestamp")).alias("gate_out_timestamp"),
                col("data.gate_id"),
